...
- 2026-07-18: Generalized PATH handling to every registered Agent. Node/npm Agents resolve `npm prefix --global` (PowerShell on native Windows, bash inside WSL, direct subprocess on POSIX) and persist that bin directory plus the user-local fallback; Hermes persists the Python user bin path. Added registry coverage and Windows npm resolution tests.
